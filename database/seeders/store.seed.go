...
func seedStores(ctx context.Context, db *pgx.Conn) {
	...
	for _, store := range stores {
		query.
			NewRow().
				...
				Set("name", store.Name).
				Set("slug", store.Slug).
				Set("short_desc", store.ShortDesc).
				Set("\"desc\"", store.Desc).
				Set("rating", store.Rating).
				Set("image", "/static/store_image_default.jpg")
	}

	fmt.Println("Seeding table stores...")
	sql, args := query.String(), query.Args()

	if _, err := db.Exec(ctx, sql, args...); err != nil {
		fmt.Println(err.Error())
		fmt.Println("Seeding table stores failed")
	} else {
		fmt.Println("Seeding table stores done!")
	}
}